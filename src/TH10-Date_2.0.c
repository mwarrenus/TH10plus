...
static void bg_layer_update(Layer *me, GContext * ctx) {
	...
	//  Keeping date between 4:00 and 5:00 is less noticeable, but constraining.
	/*
	...
	bool mins1 = mins >= (20*60)      && mins <= (24*60 + 0); // min hand blocks top date
	bool mins2 = mins >  (24*60 + 0) && mins <= (25*60 + 30); // min blocks bottom date
	bool hrs1  =  hrs >= ( 3*60 + 45) && hrs  <= ( 4*60 + 28); // hr hand blocks top date
	bool hrs2  =  hrs >  ( 4*60 + 28) && hrs  <= ( 5*60 + 25); // hr blocks bottom date
	
	if ( mins2 || (hrs1 && mins2) || ( !mins1 && !mins2 && hrs ) ){
		angle = ANGLE400;    // ~4:00 tick
		digit_set = digits4;
	} else if ( mins1 || hrs1 ){
		angle = ANGLE500;     // move date to 5:00 tick
		digit_set = digits5;
	}
	*/
...
static void handleTick(struct tm *tick_time, TimeUnits units_changed) {
	...
	// Redraw every 10 seconds (6 times/minute, 6 degrees/minute)
    /* if ((now.tm_sec % 10) == 0){
...
static void init() {
	...
	// Rotate digits[] last as it overwrites the original digitN storage
